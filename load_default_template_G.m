function [template]                     = load_default_template_G(tmp_dir,temporary_dir,par)      
% Default template: Check for an exustence of a default template in the data folder.
%                   Search criteria: lte*.txt
% Input: tmp_dir       - the directory of the saved default template (Only one can be saved there!)
%        temporary_dir - unicors temporary directory.
%        par           - unicor par structure.
%
...
% Stage 1: Find default template (if exists) and load.
    ...
    load(fullfile(tmp_dir,tmp_name),'-mat');
...
%       template.name        = [template.name '-GB' num2str(inst_broaden)];
      ...
      save([temporary_dir 'unicor_template_tmp.mat'],'template');
