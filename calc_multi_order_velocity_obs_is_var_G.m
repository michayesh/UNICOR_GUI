function [ t, v_vec, v_ind_vec, dv_vec  ,corr_poly_coeffs_vec, velocities ,corr_mat , corr_max_val_vec ,V0_data, err_msg] ...
    = calc_multi_order_velocity_obs_is_var_G(spec,tmp,obs_n,par)
...
%
% calls:
% [corr_mat(:,i), v_mat(:,i)] = spectra_cross_corr_G(tmp.wv(:,i),tmp.sp(:,i),det_x,det_y,tmp.hcv,spec.hcv,tmp.vel,i,obs_n,par);
...
if numel(good_ind)>0
    ...
    [V0_data.v, V0_data.v_ind, V0_data.dv, V0_data.c , V0_data.corr_poly_coeffs] = extract_max_corr_G(V0_data.corr_vec,velocities,par.N_parabole);
    ...
    for i = good_ind
      ...
      [v_vec(i),v_ind_vec(i), dv_vec(i), corr_max_val_vec(i),corr_poly_coeffs_vec(i,:)] = extract_max_corr_G(corr_mat(:,i),v_mat(:,i),par.N_parabole);
